...
def GeneratorBE3(z, filters, output_shape, name='G',
                num_conv=4, conv_k=3, last_k=3, repeat=0, skip_concat=False, act=lrelu, reuse=False,alternative_input_shape = False):
    with tf.variable_scope(name, reuse=reuse) as vs:
        ...
        if alternative_input_shape:
            #todo:check here if input shape is as expected based on repeat_num calculated... Now it's jsut checking that it equals 8
...
def EncoderBE3(x, filters, z_num, name='enc', num_conv=3, conv_k=3, repeat=0, act=lrelu, reuse=False, alternative_output_shape = False):
    with tf.variable_scope(name, reuse=reuse) as vs:
        x_shape = get_conv_shape(x)[1:]
        if repeat == 0:
            repeat_num = int(np.log2(np.max(x_shape[:-1]))) - 2
